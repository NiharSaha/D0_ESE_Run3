...
void ExtractQuantiles_sortFunc(int centBin, 
    const char* inputDir = "/scratch/negishi/saha115/D0_ESE_out/CMSSW_13_2_11/src/Quantiles_MB0to31_Mar30/ROOT"
) {
    ...
    while ((sFile = (TSystemFile*)next())) {
        ...
        if (!sFile->IsDirectory() && fname.EndsWith(".root") && fname.Contains("Quantiles_ESE_")) {
    ...
    if (gSystem->AccessPathName("temp_cuts_MB0to31"))  gSystem->mkdir("temp_cuts_MB0to31");
    if (gSystem->AccessPathName("temp_roots_MB0to31")) gSystem->mkdir("temp_roots_MB0to31");

    // 3. Create Output File
    TFile *fOut = new TFile(Form("temp_roots_MB0to31/hist_bin%d.root", centBin), "RECREATE");
    ...
    // Save cut text files
    std::ofstream outCut(Form("temp_cuts_MB0to31/cuts_bin%d.txt", centBin));
