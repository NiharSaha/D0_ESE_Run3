...
void ExtractQuantiles_sortFunc_new(int centBin) {
    // 1. Open input file
    TFile *fIn = TFile::Open("/scratch/negishi/saha115/D0_ESE_out/CMSSW_13_2_11/src/Quantile_PbPb2018_Feb2_with_weight_v2/ROOT/Quantiles_ESE_out_combined.root", "READ");
    if (!fIn || fIn->IsZombie()) return;

    // 2. Get the Ntuple
    TNtuple *nt = (TNtuple*)fIn->Get(Form("Q_ntuple/nt_q2_cent%i_%i", centBin, centBin+1));
    ...
    nt->SetBranchAddress(" hf_tot_w", &q2_w);
