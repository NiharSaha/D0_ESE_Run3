...
void flow_Analysis_chg(TString input_txt, TString output_path, int istart, int iend)
{
    ...
    const int min_centbin[N_CENTBINS] = {0, 10, 20, 30, 40};
    ...
    // v2/v3 vs pT per (10% cent, q-bin) — for ESE, comparison with D0 vn
    ...
    for (int ic = 0; ic < N_CENTBINS; ++ic)
        for (int iq = 0; iq < N_QBINS; ++iq)
        {
            ...
            hq2_dist[ic][iq] = new TH1D(nd2, nd2, 10000, 0.0, 1.0);
            hq3_dist[ic][iq] = new TH1D(nd3, nd3, 10000, 0.0, 1.0);
    ...
    while (file_stream >> filename)
    {
        ...
        for (Long64_t ii = 0; ii < n_entries; ii++)
        {
            ...
            int q2_idx = TMath::BinarySearch(N_QBINS + 1, q2_cuts[cent], (double)q2_hf_total);
            int q3_idx = TMath::BinarySearch(N_QBINS + 1, q3_cuts[cent], (double)q3_hf_total);

            // BinarySearch returns -1 if value < first edge: clamp to valid range
            q2_idx = std::max(0, std::min(N_QBINS - 1, q2_idx));
            q3_idx = std::max(0, std::min(N_QBINS - 1, q3_idx));
            ...
            hq2_dist[broad_cent_idx][q2_idx]->Fill(q2_hf_total);
            hq3_dist[broad_cent_idx][q3_idx]->Fill(q3_hf_total);
            ...
            for (int itrk = 0; itrk < trk_mult; ++itrk)
            {
                ...
                if (broad_cent_idx >= 0 && q2_idx >= 0 && q3_idx >= 0)
                {
                    if (res2_final > 0)
                        hp_v2[broad_cent_idx][q2_idx]->Fill(Pt, v2_obs / res2_final);
                    if (res3_final > 0)
                        hp_v3[broad_cent_idx][q3_idx]->Fill(Pt, v3_obs / res3_final);

                    // Count tracks per (cent, q-bin) vs pT for diagnostics
                    hnevt_q2[broad_cent_idx][q2_idx]->Fill(Pt);
                    hnevt_q3[broad_cent_idx][q3_idx]->Fill(Pt);
