...
void Calculate_Resolution(TString input_txt, TString output_path, int istart, int iend)
{
    ...
    Int_t min_centbin[N_CENTBINS] = {0, 5, 10, 15, 20, 25, 30, 35, 40, 45};
    Int_t max_centbin[N_CENTBINS] = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50};
    const int N_QBINS = 20;

    // Output file setup
    TString outfile = TString::Format("%s/ROOT/Resolution_%d_%d.root", output_path.Data(), istart, iend);

    TH1D *hist_Q2Q2_HFmHFp_Re_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *hist_Q2Q2_HFmHFp_Im_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *aux_hist_Q2Q2_HFmHFp = new TH1D("aux_hist_Q2Q2_HFmHFp", "", 6000, -30000, 30000);

    TH1D *hist_Q2Q2_HFmTrk_Re_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *hist_Q2Q2_HFmTrk_Im_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *aux_hist_Q2Q2_HFmTrk = new TH1D("aux_hist_Q2Q2_HFmTrk", "", 6000, -30000, 30000);

    TH1D *hist_Q2Q2_HFpTrk_Re_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *hist_Q2Q2_HFpTrk_Im_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *aux_hist_Q2Q2_HFpTrk = new TH1D("aux_hist_Q2Q2_HFpTrk", "", 6000, -30000, 30000);

    TH1D *hist_Q3Q3_HFmHFp_Re_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *hist_Q3Q3_HFmHFp_Im_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *aux_hist_Q3Q3_HFmHFp = new TH1D("aux_hist_Q3Q3_HFmHFp", "", 3000, -15000, 15000);

    TH1D *hist_Q3Q3_HFmTrk_Re_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *hist_Q3Q3_HFmTrk_Im_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *aux_hist_Q3Q3_HFmTrk = new TH1D("aux_hist_Q3Q3_HFmTrk", "", 3000, -15000, 15000);

    TH1D *hist_Q3Q3_HFpTrk_Re_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *hist_Q3Q3_HFpTrk_Im_centbin[N_CENTBINS][N_QBINS]; // centrality
    TH1D *aux_hist_Q3Q3_HFpTrk = new TH1D("aux_hist_Q3Q3_HFpTrk", "", 3000, -15000, 15000);

    TString hname;

    for (Int_t icent = 0; icent < N_CENTBINS; icent++)
    {
    ...
    while (file_stream >> filename)
    {
        ...
        for (Long64_t ii = 0; ii < n_entries; ii++)
        {
            ...
            if (cent < 0 || cent >= max_centbin[N_CENTBINS-1]) continue;
		  ...
		  int q2_bin = TMath::BinarySearch(21, q2_cuts[i_cen], (double)q2_hf_total);
                    if (q2_bin >= 0 && q2_bin < 20)
                    {
                    ...
                    int q3_bin = TMath::BinarySearch(21, q3_cuts[i_cen], (double)q3_hf_total);
                    if (q3_bin >= 0 && q3_bin < 20)
                    {
    ...
    fout->cd();
    for (Int_t i_cen = 0; i_cen < N_CENTBINS; i_cen++)
    {
