...
SCRATCH_BASE="${SCRATCH_BASE:-/scratch/negishi/saha115/D0_ESE_out/CMSSW_13_2_11/src/save_vnbinning_outputs_Mar6_v0}"
...
if [ ${#files[@]} -gt 0 ]; then
  for f in "${files[@]}"; do
    DEST="${OUT_DIR}/$(basename "${f}" .txt)_job${JOB_ID}_${TASK_ID}_$(date +%s).txt"
    mv -- "${f}" "${DEST}"
