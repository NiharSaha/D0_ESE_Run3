...
int fit_v2_prompt_dca() //updated code with q2HF (Nov 20th 2024)
{
    ...
    for (int i_pt=0; i_pt<N_PTBINS; i_pt++) {
      ...
      /*for (int i_edge=1; i_edge<N_EDGES+1; i_edge++) {
        for (int i_bin=1; i_bin<N_BINS; i_bin++) {
          if (h_v2_bin->Integral(1,i_bin)<=(i_edge*n_entries_v2/(N_EDGES*1.0)) && h_v2_bin->Integral(1,i_bin+1)>(i_edge*n_entries_v2/(N_EDGES*1.0))) { 
            cout << "edge " << i_edge << " definded: " << i_edge+7 <<"=" << h_v2_bin->GetBinLowEdge(i_bin+1) << " and " << 27-i_edge <<"="<< 1.0*h_v2_bin->GetBinLowEdge(i_bin+1) << endl;
            vnbinning[i_cen][i_pt][i_edge+7]=h_v2_bin->GetBinLowEdge(i_bin+1);
            vnbinning[i_cen][i_pt][27-i_edge]=-1.0*h_v2_bin->GetBinLowEdge(i_bin+1);
          }
  ...
  for(Long64_t i_entry=0; i_entry<N_ENTRIES; i_entry++) {
	...
	for (int i_q2=0; i_q2<N_QBINS; i_q2++) {
          if (q2_val>=q2_cuts[i_cen][i_q2] && q2_val<q2_cuts[i_cen][i_q2+1]) {
            ...
            for(int i_v2=0; i_v2<N_VBINS; i_v2++)
            {
              //if(v2_val>=vnbinning[i_cen/10][i_pt][i_v2] && v2_val<vnbinning[i_cen/10][i_pt][i_v2+1])
  ...
  for(int i_pt=0; i_pt<N_PTBINS; i_pt++) {
    for(int i_cen=0; i_cen<N_CENTBINS_1; i_cen++) {
      ...
      /*h_mass_default[i_cen/10][i_pt]->Add(h_mass_default_1percent_cen[i_cen][i_pt]);
      for(int i_q2=0; i_q2<N_QBINS; i_q2++) {
        h_mass[i_cen/10][i_q2][i_pt]->Add(h_mass_1pecent_cen[i_cen][i_q2][i_pt]);
        for(int i_v2=0; i_v2<N_VBINS; i_v2++)
        {
          h_mass_v2_fit[i_cen/10][i_q2][i_pt][i_v2]->Add(h_mass_v2_fit_1pecent_cen[i_cen][i_q2][i_pt][i_v2]);
        }
	}*/
  ...
  for (int i_cen=0; i_cen<N_CENTBINS; i_cen++) {
    for (int i_pt=0; i_pt<N_PTBINS; i_pt++) {
      ...
      for (int i_q2=0; i_q2<N_QBINS; i_q2++) {
        ...
        //tex->DrawLatex(0.14,0.86,Form("cent %s pT %s GeV/c q2 %d",cen_label[i_cen].Data(),pt_label[i_pt].Data(),i_q2));
        ...
        leg->SetTextFont(42);
        leg->SetFillStyle(0);
        //leg->AddEntry(h_mass[i_cen][i_q2][i_pt]," Data","lep"); //for full q2
        ...
        //c1->SaveAs(Form("prompt_default_fit_plots/massplot_cen_%s_q2bin_%d_pt_%s.pdf",cen_name[i_cen].Data(),i_q2,pt_name[i_pt].Data()));
        ...
        for (int i_v2=0; i_v2<N_VBINS; i_v2++) {
          ...
          //c1->SaveAs(Form("prompt_mass_plot_withchi2_sigma/hmassfit_pt_%s_cen%s_q2bin_%d_v2bin_%d.pdf",pt_name[i_pt].Data(),cen_name[i_cen].Data(),i_q2,i_v2));
  ...
  TFile *outf = new TFile("prompt_vn_in_q2HF_total_graph_fullrange_withchi2_sigma_vs_v2.root","recreate");
  outf->cd();
  ...
  for (int i_cen=0; i_cen<N_CENTBINS; i_cen++) {
	...
	chi2_ndf_for_q2_v2_graph[i_cen][i_q2][i_pt]->SetNameTitle(
								  ("chi2ndf_graph_pt" + pt_name[i_pt] + "_cen" + cen_name[i_cen] + "_q2bin_" + to_string(i_q2)).c_str(),
								  ("chi2ndf_graph_pt" + pt_name[i_pt] + "_cen" + cen_name[i_cen] + "_q2bin_" + to_string(i_q2)).c_str()
								  );
