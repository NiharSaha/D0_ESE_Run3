...
int save_vnbinning_to_txt(int target_cen_group = -1, int target_pt = -1)
{
  ...
  Int_t cen_edges[N_CENTBINS+1]={0, 10, 30, 50};
  static const char* cen_name[N_CENTBINS] = {"cent0to10", "cent10to30", "cent30to50"};
...
int main(int argc, char **argv)
{
  if (argc==1) {
    ...
    int idx = atoi(argv[1]);
    if (idx < 0 || idx > 2) {
      std::cerr << "ERROR: target_cen_group must be 0-2, got " << idx << std::endl;
      return 1;
    }
    ...
    if (cen_idx < 0 || cen_idx > 2) {
      std::cerr << "ERROR: target_cen_group must be 0-2, got " << cen_idx << std::endl;
      return 1;
    }
