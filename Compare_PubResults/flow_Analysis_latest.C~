  ...
  file_res->Close();
	...
	for (Long64_t ii = 0; ii < n_entries; ii++) {
	    ...
	    if(ii % 100000 == 0)
	      ...
	      // --- DIAGNOSTIC: Check BDT application for pT 20-40 GeV/c ---
	      /*if (Pt >= 20.0 && Pt < 40.0) {
		static Long64_t n_before_bdt_20to40 = 0;
		static Long64_t n_after_bdt_20to40  = 0;
		static Long64_t n_printed = 0;
		n_before_bdt_20to40++;
		if (Mva > bdt_cut) {
		  n_after_bdt_20to40++;
		  if (n_printed < 20) {
		    printf("[BDT-CHECK pT20-40] icand=%d | Pt=%.3f | Y=%.3f | cent=%d (2*cent=%d) | Mva=%.6f | bdt_cut=%.6f | PASS\n",
			   icand, Pt, Y, cent, 2*cent, Mva, bdt_cut);
		    n_printed++;
		  }
		} else {
		  if (n_printed < 20) {
		    printf("[BDT-CHECK pT20-40] icand=%d | Pt=%.3f | Y=%.3f | cent=%d (2*cent=%d) | Mva=%.6f | bdt_cut=%.6f | FAIL (cut applied)\n",
			   icand, Pt, Y, cent, 2*cent, Mva, bdt_cut);
		    n_printed++;
		  }
		}
		// Print summary every 10000 candidates in this pT range
		if (n_before_bdt_20to40 % 10000 == 0) {
		  printf("[BDT-SUMMARY pT20-40] Before: %lld | After: %lld | Efficiency: %.4f\n",
			 n_before_bdt_20to40, n_after_bdt_20to40,
			 (n_before_bdt_20to40 > 0) ? (double)n_after_bdt_20to40/n_before_bdt_20to40 : 0.0);
		}
	      }
	      // --- END DIAGNOSTIC ---
	      */
